...
const ModelSempor = () => {
  ...
  useEffect(() => {
    const map = new maplibregl.Map({
      ...
      style: {
        ...
        sources: {
          ...
          alatSempor: {
            type: "geojson",
            data: "http://103.176.97.201:8080/geoserver/serayu_sempor/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=serayu_sempor%3Asensor_sempor&outputFormat=application%2Fjson",
          },
          batasWadukSempor: {
            type: "geojson",
    ...
    map.on("load", () => {
      ...
      map.on("click", "alat-sempor-layer", (e) => {
        ...
        console.log("Total features in same coord:", sameCoordFeatures);

        const popupContent = sameCoordFeatures
          .map((f) => {
            const { id, nama_sensor, nmfield, satuan, keterangan, value } =
              f.properties;

            return `
              <div style="margin-bottom: 8px;">
                <div><strong>${id}-Sensor</strong> : ${nama_sensor}</div>
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          "http://103.176.97.201:8080/geoserver/serayu_sempor/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=serayu_sempor%3Asensor_sempor&outputFormat=application%2Fjson"
        );
        ...
        console.log("filter data", filteredFeatures);
  ...
  return (
    <div className="relative w-full h-screen">
      ...
      <div
        className={`bg-white top-5 left-[9px] absolute w-60 h-[calc(100vh-2.5rem)] transition-transform duration-500 ease-in-out rounded-md ${
          isDataLayerVisible
            ? "translate-x-0"
      ...
      >
        ...
        <div className="p-3 text-[#333333] space-y-3 overflow-y-auto max-h-[calc(100%-3rem)] rounded-b-md">
          {alatSemporData.map((item) => {
            ...
            const { keterangan } = item.properties;
